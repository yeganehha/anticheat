...
ClientConfig.GeneralStuff = false -- Includes: NO INFINITE AMMO, NO GODMODE, NO ANTI RUNOFF


ClientConfig.ShowIdOnPage = true -- Includes: NO INFINITE AMMO, NO GODMODE, NO ANTI RUNOFF

--//Anti GodMode//--
ClientConfig.AntiGodMode = false -- Master switch for AntiGodMode
ClientConfig.AntiGodModeTimer = 50000 -- AntiGodMode Timer, default: 200000 / 20 seconds, it does the check every x ms
ClientConfig.MaxPlayerHealth = 201 -- Max player health
ClientConfig.AntiGodModeKick = false -- Kick the AntiGodMode detected player
ClientConfig.AntiGodModeBan = false -- Ban the AntiGodMode detected player

ClientConfig.PlayerProtection = false -- FIRE AND EXPLOSIONS (this only protect your players from being damaged by explosions or fire)

--//Anti Speed//--
ClientConfig.AntiSpeedHack = false -- On foot (not falling) anti speed hack (optional)
ClientConfig.SpeedHackValue = 10.0 -- MAX SPEED ALLOWED FOR WALKING/RUNNING/SWIMMING from 0.0 to 99.0


--//Anti AdminStuff//--
ClientConfig.AntiPlayerBlips = false -- Detects a user that activated Player Blips and is not allowed
ClientConfig.AntiSpectate = false -- Detects a user that is spectating someone else and is not allowed

--//Anti Blacklisted Weapons//--
ClientConfig.BlacklistedWeapons = false -- Master switch for BlacklistedWeapons (detects who's holding a blacklisted weapon)
ClientConfig.BlacklistedWeaponsTable = { -- Table for all blacklisted Weapons (don't forget the comma! bitch)
...
--//Anti Cheat Engine//--
ClientConfig.AntiDamageModifier = false -- Detects if a player tries to modify his damage or defence value
ClientConfig.AntiDamageChanger = false -- Detects if a player tries to modify his damage or defence value

ClientConfig.WeaponExplosiveCheck = false -- Detects if a player tries to explosive bullt 
...
ClientConfig.AntiVehicleHashChanger = true -- Detects if a player tried to change his vehicle hash model


ClientConfig.AntiModelChanger = false -- Detects if a player tried to change his model to a blacklisted model
ClientConfig.AntiModelChangerTable = {
	"s_m_y_swat_01",
	"a_m_y_mexthug_01", 
    "a_c_cat_01", 
    "a_c_boar", 
